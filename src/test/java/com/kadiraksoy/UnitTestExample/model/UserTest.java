...
class UserTest {
    ...
    void testGetters(){
        ...
        List<Todo> actualTodo = user.getTodos();

        assertEquals(1L,actualId);
        assertEquals("testUser",actualUsername);
        assertEquals(2,actualTodo.size());
        assertEquals("Todo 1", actualTodo.get(0).getDescription());
        assertEquals("Todo 2", actualTodo.get(1).getDescription());
    }
